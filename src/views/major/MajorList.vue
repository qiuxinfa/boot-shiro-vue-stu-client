	...
	<el-row>
		...
		<el-col :span="64" class="toolbar" style="padding-bottom: 0px;with:100%;height:100%">
			<el-form :inline="true" :model="filters" ref="filters">
				<el-form-item>
					<el-input placeholder="专业名称" v-model="filters.keyword1"></el-input>
				</el-form-item>
				<el-form-item>
...
<!-- 新增属性 -->
	<el-dialog title="新增专业" :visible.sync="dialogFormVisible1">
	...
	<el-form-item label="所属学院" prop="instituteId2">
		<el-select v-model="attr.instituteId2" filterable placeholder="请选择">
			<el-option
	...
	<el-form-item label="专业名称" prop="name2" :rules="[{ required: true, message: '请输入专业名称', trigger: 'blur' }]">
		<el-input  type="text" v-model="attr.name2" placeholder="请输入专业名称" auto-complete="off"></el-input>
	</el-form-item>
	<el-form-item label="专业代码" prop="majorNumber2" :rules="[{ required: true, message: '请输入专业代码', trigger: 'blur' }]">
		<el-input  type="text" v-model="attr.majorNumber2" placeholder="请输入专业代码" auto-complete="off"></el-input>
	</el-form-item>	
...
export default {
	...
	methods: {
		...
		async getFormData1 () {
			...
			let data = await http.get('major/list', params)
		...
		async addInstitute() {
			...
			let params = {
				...
				majorNumber: _this.attr.majorNumber2,
				instituteId: _this.attr.instituteId2
			}
			let data = await http.post("major/add", params)
		...
		async delAttr () {
			if (this.attrIds.length === 0) {
				 this.message(true,'请选择需要删除的专业','warning')
				 return
			}
      ...
      let _this = this
			...
			let data =await http.post('major/delete', params)
