	  ...
	  <!-- 查看学生 -->
			<el-table-column
			      ...
			      <template slot-scope="scope">
			      	<el-button
							  @click.native="viewStu(scope.row.id)"
			          type="text"
			          size="small">
			          查看学生
			        </el-button>
...
<!-- 录入成绩 -->
	<el-dialog title="录入成绩" :visible.sync="dialogFormVisible1">
	  <div style="width:60%;margin: 0 auto">
		<el-form ref="attr" :model="attr" :inline="false" label-width="90px" class="demo-ruleForm">
			<el-form-item label="录入成绩" prop="score2" :rules="[{ required: true, message: '请录入成绩', trigger: 'blur' },{validator: 'regexp', pattern: /^(?:[1-9]?\d|100)$/, message: '成绩只能是1-100之间的整数', trigger: 'change,blur'}]">
				<el-input  type="number" v-model="attr.score2" placeholder="请录入成绩" auto-complete="off"></el-input>
			</el-form-item>		
		</el-form>
	 </div>
   ...
   <!-- 属性明细 -->
  <el-dialog title="选课的学生" :visible.sync="dialogTableVisible1">
...
<!-- 		 <el-form :inline="true" label-width="90px" class="demo-ruleForm">
	  ...
	  <el-table :data="tableData2" @selection-change="handleSelectionChange2">
		...
		<el-table-column property="studentName" label="成绩">
			<template slot-scope="scope">
				<span v-if="scope.row.status=='1'">{{scope.row.score}}</span>
				<span v-else>尚未录入成绩</span>
			</template>
		</el-table-column>
		<!-- 录入成绩 -->
		<el-table-column
		      ...
		      <template slot-scope="scope">
		      	<el-button v-if="scope.row.status=='0'"
						  @click.native="preUpdateScore(scope.row.id)"
		          type="text"
		          size="small">
		          录入成绩
		        </el-button>
...
export default {
	...
	methods: {
		...
		async getFormData1 () {
			...
			let data = await http.get('course/center', params)
		...
		async viewStu(ctId){
				...
				let data = await http.get('grade/viewStudent',param)
				if(!data.data) {
					return
				}
				if (data.data.status === 200) {
		...
		async updateScore(){
				...
				let param = {
					id: _this.gid,
					score: _this.attr.score2,
					status:'1'
				}
				let data = await http.post('grade/update',param)
				if(!data.data) {
					return
				}
